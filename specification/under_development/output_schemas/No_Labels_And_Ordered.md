...
For systems that don't support string parameters, the ordered output format can be used. This format omits any labels that are passed through output recording functions.

The format specifies what the [output-recording][] functions emit based on the selected schema and entry point [attributes][].
...
- One `START` record
- At least one `METADATA` record. The generation of these records is not part of the generated QIR, but written as part of the [base profile][] spec as pass-thru metadata. Likewise the `START` and `END` records are not part of the generated QIR. The `output_labeling_schema` flows through the metadata records.
- One or more `OUTPUT` records
- One `END` record

The `START` record does not have any value entry. The `END` record has a value entry of `0`. The `RESULT` records will have a value corresponding to the runtime function called, either a result type record or a string representation of a supported primitive type. The `OUTPUT` records must appear in the same order as the calls in the QIR program. 
...
[output recording]: ../profiles/Base_Profile.md#output-recording
